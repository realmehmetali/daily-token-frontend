...
import { HUB, APP_ID, CHAIN_ID as WORLDCHAIN_ID, ACTION_CLAIM } from "@/lib/env";
...
export default function DailyTokenClaimOnly() {
  ...
  async function handleClaimWithMiniKit() {
    try {
      if (!MiniKit.isInstalled()) {
        alert("Open inside World App.");
        return;
      }
      if (!address) {
      ...
      // 1) Ask World App to generate proof for your claim action
      ...
      if (finalPayload.status === "error") {
        ...
        const msg =
  (typeof finalPayload === "object" && finalPayload && "message" in finalPayload
    ? (finalPayload as any).message
    : (typeof finalPayload === "object" && finalPayload && "error" in finalPayload
...
alert(`World ID error: ${msg}`);
      ...
      // 2) Verify proof on your backend (per docs)
      ...
      if (json.status !== 200 || !json.verifyRes?.success) {
        console.error("verify API failed:", json);
        alert("Proof verification failed.");
        setClaiming(false);
        return;
      }

      // 3) On success, do your on-chain tx (same as before)
      ...
      console.error("claim flow failed:", err);
    ...
    <div className="min-h-screen pb-24" style={{ background: theme.bg, color: theme.text }}>
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 space-y-6">
        ...
        <section className="rounded-3xl border p-5" style={{ background: "#FFFFFF", borderColor: theme.border }}>
          ...
          {/* Step 2 — CLAIM (MiniKit.verify + on-chain tx) */}
          <div className="mt-3">
            <button
              onClick={async () => {
                if (chainId !== WORLDCHAIN_ID) {
                  await ensureWorldchain().catch(() => {});
                }
                await handleClaimWithMiniKit();
              }}
